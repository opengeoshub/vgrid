...
# def fix_isea3h_antimeridian_cells(isea3h_boundary, threshold=0):
#     """
#     Adjusts polygon coordinates to handle antimeridian crossings.
#     """
#     lon_lat = [(float(lon), float(lat)) for lon, lat in isea3h_boundary.exterior.coords]
...
def cell_to_polygon(eaggr_cell):
    cell_to_shape =  eaggr_dggs.convert_dggs_cell_outline_to_shape_string(eaggr_cell, ShapeStringFormat.WKT)
    if cell_to_shape:
