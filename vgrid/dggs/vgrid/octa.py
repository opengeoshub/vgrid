...
def calculate_trapezoid_subdivision_latitude(
    lat1: float, lon1: float, lon2: float, level: int
) -> float:
    """
...
def calculate_triangle_subdivision_latitudes(
    lat_pole: float, lon1: float, lon2: float, level: int
) -> tuple[float, float]:
    """
...
def subdivide_triangle_pattern(cell: Cell, level: int, max_level: int) -> List[Cell]:
    ...
    if is_north_pole:
        children = [
            ...
            Cell(
                cell.lambda1, cell.lambda2, cell.phi1, phi2, phi3, cell.quad_code + "1"
            ),
            # Left-bottom trapezoid (SW) - code "2"
            Cell(
                cell.lambda1, cell.lambda2, cell.phi1, phi2, phi3, cell.quad_code + "2"
            ),
            # Right-bottom trapezoid (SE) - code "3"
            Cell(
                cell.lambda1, cell.lambda2, cell.phi1, phi2, phi3, cell.quad_code + "3"
            ),
        ]
    else:
        children = [
...
def subdivide_cell(cell: Cell, level: int, max_level: int) -> List[Cell]:
    ...
    children = [
        Cell(
            cell.lambda1, cell.lambda2, cell.phi1, phi2, phi3, cell.quad_code + "0"
        ),  # SW
        Cell(
            cell.lambda1, cell.lambda2, cell.phi1, phi2, phi3, cell.quad_code + "1"
        ),  # SE
...
def calculate_cell_area(cell: Cell) -> float:
    ...
    area = (
        ...
        * EARTH_RADIUS
        * lon_diff_rad
        * (math.sin(lat_max_rad) - math.sin(lat_min_rad))
    )
    return area
