...
BADLY_FORMED = "An MGRS string error: string too long, too short, or badly formed"
...
LATITUDE_BANDS = [
    (ALPHABET["C"], 1100000.0, -72.0, -80.5, 0.0),
...
def _log_proj_crs(proj_crs, proj_desc="", espg=""):
    ...
    log.debug("{0}proj: {1}{2}{3}".format(proj_desc, espg, os.linesep, definition))
...
def toWgs(mgrs):
    ...
    longitude, latitude = _transform(easting, northing, epsg, 4326, polar=(not utm))
...
def _breakMgrsString(mgrs):
    ...
    # MGRS letters
    count = sum(
        1 for _ in itertools.takewhile(str.isalpha, itertools.islice(mgrs, idx, None))
    )
    if count == 3:
        a = ord("A")
    ...
    count = sum(
        1 for _ in itertools.takewhile(str.isdigit, itertools.islice(mgrs, idx, None))
    )
...
def _clean_mgrs_str(s):
    ...
    if str(type(s)) == "<class 'bytes'>":  # Py 3
        ...
        s = str(s, encoding="UTF-8")  # <type 'unicode'>
...
# Added by Vgrid
################
def get_mgrs_resolution_and_cell_size(mgrs_id):
    """Get the resolution and cell size of the MGRS code based on its length.

    @param mgrs: MGRS coordinate string (no spaces)
    @returns: tuple containing (resolution in meters, grid size in meters)
    """
    # Clean the MGRS string (if necessary)
    mgrs_id = _clean_mgrs_str(mgrs_id)
...
#     resolution, grid_size = get_mgrs_resolution_and_cell_size(mgrs_id)
#     zone, hemisphere, easting, northing = _mgrsToUtm(mgrs_id)
...
#     return min_lat, min_lon, max_lat, max_lon, resolution
...
def mgrscell(mgrs_id):
    ...
    min_x, min_y = easting, northing
    max_x, max_y = (min_x + grid_size, min_y + grid_size)

    epsg = _epsgForUtm(zone, hemisphere)

    min_lon, min_lat = _transform(min_x, min_y, epsg, 4326, polar=False)
    max_lon, max_lat = _transform(max_x, max_y, epsg, 4326, polar=False)
