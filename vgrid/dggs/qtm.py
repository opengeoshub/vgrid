...
# Base octahedral faces for QTM (IDs 1-8); canonical source for qtm_id_to_facet.
QTM_INITIAL_FACETS = [
...
QTM_INITIAL_FACETS_BY_ID = {
    str(i + 1): facet for i, facet in enumerate(QTM_INITIAL_FACETS)
}


def qtm_id_to_facet(qtm_id):
    base_facet = QTM_INITIAL_FACETS_BY_ID.get(qtm_id[0])
...
def _pick_subfacet_index(subfacets, point):
    """Return the child facet index for a point (boundary- and gap-safe)."""
    for i, subfacet in enumerate(subfacets):
        sub_geom = constructGeometry(subfacet)
        if sub_geom.contains(point) or sub_geom.covers(point):
            return i

    matches = []
    for i, subfacet in enumerate(subfacets):
        sub_geom = constructGeometry(subfacet)
        if sub_geom.intersects(point) or sub_geom.touches(point):
            matches.append((sub_geom.area, i))
    if matches:
        return min(matches)[1]

    # Subdivision geometries can leave tiny gaps at deep levels.
    nearest = None
    for i, subfacet in enumerate(subfacets):
        dist = constructGeometry(subfacet).distance(point)
...
def latlon_to_qtm_id(lat, lon, resolution):
    ...
    for facet_id, facet in QTM_INITIAL_FACETS_BY_ID.items():
        facet_geom = constructGeometry(facet)
        if facet_geom.contains(point) or facet_geom.covers(point):
            qtm_id = facet_id
            current_facet = facet
            break
    else:
    ...
    for _ in range(1, resolution):
        ...
        idx = _pick_subfacet_index(subfacets, point)
        if idx is None:
            raise ValueError(
                f"Could not locate ({lat}, {lon}) in QTM facet at level {len(qtm_id)}"
            )
        qtm_id += str(idx)
        current_facet = subfacets[idx]
