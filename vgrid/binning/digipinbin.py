...
Key Functions:
- digipin_bin(): Core binning function with spatial joins and aggregation
- digipinbin(): Main user-facing function with multiple input/output formats
- digipinbin_cli(): Command-line interface for binning functionality
"""
...
def digipin_bin(
    ...
    **kwargs,
    ...
    # Generate DIGIPIN grid covering the points' bbox
    minx, miny, maxx, maxy = points_gdf.total_bounds
    id_col = "digipin"
    from vgrid.generator.digipingrid import digipin_grid

    grid_gdf = digipin_grid(resolution=resolution, bbox=(minx, miny, maxx, maxy))
...
def digipinbin(
    ...
    **kwargs,
    ...
    result_gdf = digipin_bin(data, resolution, stats, category_col, numeric_col, **kwargs)
    output_name = None
    if output_format in OUTPUT_FORMATS:
        ...
        if isinstance(data, str):
            ...
            output_name = f"{base}_digipinbin_{resolution}"
        else:
            output_name = f"digipinbin_{resolution}"
    return convert_to_output_format(result_gdf, output_format, output_name)


def digipinbin_cli():
    parser = argparse.ArgumentParser(description="Binning point data to DIGIPIN DGGS")
    parser.add_argument(
        "-i",
        "--input",
        type=str,
