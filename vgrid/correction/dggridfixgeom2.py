...
def fix_geojson(geojson_data, threshold=-128):
    ...
    for feature in geojson_data.get("features", []):
        ...
        if geometry["type"] == "Polygon":
            ...
            geometry["coordinates"][0] = fix_antimeridian_cells(geometry["coordinates"][0], threshold)
            # Fix any interior rings (holes)
            for i in range(1, len(geometry["coordinates"])):
                geometry["coordinates"][i] = fix_antimeridian_cells(geometry["coordinates"][i], threshold)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fix antimeridian GeoJSON created from DGGRID")
    ...
    output_file = os.path.join(os.getcwd(), f"{os.path.splitext(os.path.basename(args.input))[0]}_geom_fixed_2.geojson")
