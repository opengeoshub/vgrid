...
def _raster2gars_binning(
    ...
    if not properties:
        return gpd.GeoDataFrame(columns=["gars", "geometry"], crs="EPSG:4326")
...
def raster2gars_cli():
    parser = argparse.ArgumentParser(
        description="Convert raster to GARS DGGS (EPSG:4326 cells)"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help=f"GARS resolution [{min_res}..{max_res}]. Omit to infer from pixel size.",
    )
    parser.add_argument(
        "-m",
        "--method",
        ...
        default="binning",
        help="binning: aggregate pixels into GARS cells; nearest_neighbour: nearest pixel center",
    )
    parser.add_argument(
        "-f",
        "--output_format",
