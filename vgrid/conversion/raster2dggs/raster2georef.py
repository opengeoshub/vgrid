...
def _georef_polygon(georef_id):
    cell = georef2geo(georef_id)
...
def _raster2georef_binning(
    ...
    for georef_id, acc in tqdm(
        ...
        if cell_polygon is None:
            continue
        base_props = {"georef": georef_id, "geometry": cell_polygon}
    ...
    if not properties:
        return gpd.GeoDataFrame(columns=["georef", "geometry"], crs="EPSG:4326")
...
def raster2georef_cli():
    parser = argparse.ArgumentParser(
        description="Convert raster to GEOREF DGGS (EPSG:4326 cells)"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help=f"GEOREF resolution [{min_res}..{max_res}]. Omit to infer from pixel size.",
    )
    parser.add_argument(
        "-m",
        "--method",
        ...
        default="binning",
        help="binning: aggregate pixels into GEOREF cells; nearest_neighbour: nearest pixel center",
    )
    parser.add_argument(
        "-f",
        "--output_format",
