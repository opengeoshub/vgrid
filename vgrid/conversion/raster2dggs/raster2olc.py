...
def _raster2olc_nearest_neighbour(raster_path, resolution,
    verbose=True,
) -> gpd.GeoDataFrame:
    footprint = footprint_gdf_from_raster(raster_path)
    grid_gdf = generate_grid(footprint, "olc", resolution, verbose=verbose)
    return nearest_neighbour_from_grid(raster_path, grid_gdf, verbose=verbose)


def _raster2olc_binning(raster_path, resolution, stats,
    verbose=True,
) -> gpd.GeoDataFrame:
    def cell_id(lat, lon):
    ...
    olc_acc, band_count = accumulate_raster_pixels(
        raster_path, cell_id, stats, desc="Binning raster blocks to OLC", verbose=verbose
    )
...
def raster2olc(
    ...
    if method == "binning":
        ...
        print(f"Stats: {stats}")
        gdf = _raster2olc_binning(raster_path, resolution, stats, verbose=verbose)
    else:
        gdf = _raster2olc_nearest_neighbour(raster_path, resolution, verbose=verbose)
