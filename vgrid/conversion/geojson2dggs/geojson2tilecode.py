...
def poly_to_grid(resolution, geometry, feature_properties, compact):
    ...
    for poly in polys:
        ...
        tilecode_ids = []
        for tile_code in tilecodes:
            tilecode_id = f"z{tile_code.z}x{tile_code.x}y{tile_code.y}"
    ...
    tilecode_geosjon = {"type": "FeatureCollection", "features": tilecode_features}
...
def geojson2tilecode_cli():
    ...
    try:
        ...
        if args.compact:
            geojson_path = (
                f"{geojson_name}2tilecode_{args.resolution}_compacted.geojson"
            )
