...
def point2isea3h(isea3h_dggs, resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    isea3h_features = []
...
def polyline2isea3h(isea3h_dggs, resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    isea3h_features = []
...
def polygon2isea3h(isea3h_dggs, resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    isea3h_features = []
...
def geometry2isea3h(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    if not is_windows():
    ...
    for idx, geom in enumerate(tqdm(geometries, desc="Processing features")):
        ...
        if geom.geom_type == 'Point':
            isea3h_features.extend(point2isea3h(isea3h_dggs, resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                isea3h_features.extend(point2isea3h(isea3h_dggs, resolution, pt, props))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            isea3h_features.extend(polyline2isea3h(isea3h_dggs, resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            isea3h_features.extend(polygon2isea3h(isea3h_dggs, resolution, geom, props, predicate, compact, topology, include_properties))
    return {"type": "FeatureCollection", "features": isea3h_features}

def dataframe2isea3h(df, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2isea3h(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2isea3h(gdf, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2isea3h(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):
    if not is_windows():
        raise NotImplementedError("ISEA3H DGGS conversion is only supported on Windows")
    ...
    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        ...
        result = dataframe2isea3h(data, resolution, predicate, compact, topology, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2isea3h(data, resolution, None, predicate, compact, topology, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2isea3h(gdf, resolution, predicate, compact, topology, include_properties)
...
def vector2isea3h_cli():
    parser = argparse.ArgumentParser(description='Convert vector data to ISEA3H grid cells (Windows only)')
    parser.add_argument('-i', '--input', help='Input file path, URL')
    parser.add_argument('-r', '--resolution', type=int, choices=range(33), metavar='[0-32]', help='ISEA3H resolution [0..32] (0=coarsest, 32=finest)')
    ...
    parser.add_argument('-c', '--compact', action='store_true', help='Enable ISEA3H compact mode for polygons')
