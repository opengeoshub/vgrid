...
def point_to_olc(resolution, point, feature_properties=None):
    olc_features = []
...
def poly_to_olc(resolution, geometry, feature_properties=None, compact=False):
    olc_features = []
...
def geometry2olc(geometries, resolution, properties_list=None, compact=False, include_properties=True):
    olc_features = []
    for idx, geom in enumerate(geometries):
        ...
        if geom.geom_type == 'Point':
            olc_features.extend(point_to_olc(resolution, geom, props))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                olc_features.extend(point_to_olc(resolution, pt, props))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            olc_features.extend(poly_to_olc(resolution, geom, props, compact))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            olc_features.extend(poly_to_olc(resolution, geom, props, compact))
    return {"type": "FeatureCollection", "features": olc_features}

def dataframe2olc(df, resolution, compact=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2olc(geometries, resolution, properties_list, compact, include_properties)

def geodataframe2olc(gdf, resolution, compact=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2olc(data, resolution, compact=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):
    allowed_resolutions = [2, 4, 6, 8, 10, 11, 12, 13, 14, 15]
    if not isinstance(resolution, int) or resolution not in allowed_resolutions:
    ...
    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        result = geodataframe2olc(data, resolution, compact, include_properties)
    elif isinstance(data, pd.DataFrame):
        result = dataframe2olc(data, resolution, compact, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2olc(data, resolution, None, compact, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2olc(gdf, resolution, compact, include_properties)
