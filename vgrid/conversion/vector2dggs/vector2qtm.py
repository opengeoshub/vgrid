...
def point2qtm(resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    qtm_features = []
    latitude = point.y
    longitude = point.x
...
def polyline2qtm(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    qtm_features = []
...
def polygon2qtm(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    qtm_features = []
                ...
                for i, pf in enumerate(levelFacets[lvl - 1]):
                    ...
                    for j, subfacet in enumerate(subdivided_facets):
                        ...
                        if Polygon(subfacet_geom).intersects(polygon):
                            ...
                            levelFacets[lvl].append(subfacet)
                            if lvl == resolution - 1:
                                if not check_predicate(Polygon(subfacet_geom), polygon, predicate):
                                    continue
                                num_edges = 3
                                qtm_feature = geodesic_dggs_to_feature("qtm", new_id, resolution, subfacet_geom, num_edges)
                                if include_properties and feature_properties:
...
def geometry2qtm(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    ...
    for idx, geom in enumerate(tqdm(geometries, desc="Processing features")):
        ...
        if geom.geom_type == 'Point':
            qtm_features.extend(point2qtm(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                qtm_features.extend(point2qtm(resolution, pt, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            qtm_features.extend(polyline2qtm(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            qtm_features.extend(polygon2qtm(resolution, geom, props, predicate, compact, topology, include_properties))
    return {"type": "FeatureCollection", "features": qtm_features}

def dataframe2qtm(df, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2qtm(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2qtm(gdf, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2qtm(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):

    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        result = geodataframe2qtm(data, resolution, predicate, compact, topology, include_properties)
    elif isinstance(data, pd.DataFrame):
        result = dataframe2qtm(data, resolution, predicate, compact, topology, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2qtm(data, resolution, None, predicate, compact, topology, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2qtm(gdf, resolution, predicate, compact, topology, include_properties)
