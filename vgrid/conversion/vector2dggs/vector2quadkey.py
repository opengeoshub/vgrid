...
def validate_quadkey_resolution(resolution):
    ...
    if resolution < 0 or resolution > 29:
        raise ValueError(f"Resolution must be in range [0..29] (0=coarsest, 29=finest), got {resolution}")
    
    return resolution

def point2quadkey(resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    quadkey_features = []
...
def polyline2quadkey(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    quadkey_features = []
...
def polygon2quadkey(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    quadkey_features = []
...
def geometry2quadkey(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    ...
    for idx, geom in tqdm(enumerate(geometries), desc="Processing features"):
        ...
        if geom.geom_type == 'Point':
            quadkey_features.extend(point2quadkey(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                quadkey_features.extend(point2quadkey(resolution, pt, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            quadkey_features.extend(polyline2quadkey(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            quadkey_features.extend(polygon2quadkey(resolution, geom, props, predicate, compact, topology, include_properties))
    return {"type": "FeatureCollection", "features": quadkey_features}

def dataframe2quadkey(df, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2quadkey(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2quadkey(gdf, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2quadkey(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):
    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        result = geodataframe2quadkey(data, resolution, predicate, compact, topology, include_properties)
    elif isinstance(data, pd.DataFrame):
        result = dataframe2quadkey(data, resolution, predicate, compact, topology, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2quadkey(data, resolution, None, predicate, compact, topology, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:                                
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2quadkey(gdf, resolution, predicate, compact, topology, include_properties)
