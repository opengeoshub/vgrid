...
def point2tilecode(resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    tilecode_features = []
...
def polyline2tilecode(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    tilecode_features = []
...
def polygon2tilecode(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    tilecode_features = []
...
def geometry2tilecode(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    ...
    for idx, geom in tqdm(enumerate(geometries), desc="Processing features"):
        ...
        if geom.geom_type == 'Point':
            tilecode_features.extend(point2tilecode(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                tilecode_features.extend(point2tilecode(resolution, pt, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            tilecode_features.extend(polyline2tilecode(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            tilecode_features.extend(polygon2tilecode(resolution, geom, props, predicate, compact, topology, include_properties))
    return {"type": "FeatureCollection", "features": tilecode_features}

def dataframe2tilecode(df, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2tilecode(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2tilecode(gdf, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2tilecode(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):

    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        result = geodataframe2tilecode(data, resolution, predicate, compact, topology, include_properties)
    elif isinstance(data, pd.DataFrame):    
        result = dataframe2tilecode(data, resolution, predicate, compact, topology, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2tilecode(data, resolution, None, predicate, compact, topology, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2tilecode(gdf, resolution, predicate, compact, topology, include_properties)
