...
def point2mgrs(resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    mgrs_features = []
...
def polyline2mgrs(resolution, geometry, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    # Placeholder: MGRS polyline/polygon support not implemented in geojson2mgrs.py
    return []

def polygon2mgrs(resolution, geometry, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    # Placeholder: MGRS polyline/polygon support not implemented in geojson2mgrs.py
    return []


def geometry2mgrs(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    ...
    for idx, geom in tqdm(enumerate(geometries), desc="Processing features"):
        ...
        if geom.geom_type == 'Point':
            mgrs_features.extend(point2mgrs(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type == 'MultiPoint':
            for pt in geom.geoms:
                mgrs_features.extend(point2mgrs(resolution, pt, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('LineString', 'MultiLineString'):
            mgrs_features.extend(polyline2mgrs(resolution, geom, props, predicate, compact, topology, include_properties))
        elif geom.geom_type in ('Polygon', 'MultiPolygon'):
            mgrs_features.extend(polygon2mgrs(resolution, geom, props, predicate, compact, topology, include_properties))

    return {"type": "FeatureCollection", "features": mgrs_features}

def dataframe2mgrs(df, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
    ...
    return geometry2mgrs(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2mgrs(gdf, resolution, predicate=None, compact=False, topology=False, include_properties=True):
    geometries = []
    properties_list = []
...
def vector2mgrs(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):

    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        result = geodataframe2mgrs(data, resolution, predicate, compact, topology, include_properties)
    elif isinstance(data, pd.DataFrame):
        result = dataframe2mgrs(data, resolution, predicate, compact, topology, include_properties)
    elif hasattr(data, 'geom_type') or (isinstance(data, list) and len(data) > 0 and hasattr(data[0], 'geom_type')):
        result = geometry2mgrs(data, resolution, None, predicate, compact, topology, include_properties)
    elif isinstance(data, dict) and 'type' in data:
        try:
            gdf = gpd.GeoDataFrame.from_features(data['features'])
            result = geodataframe2mgrs(gdf, resolution, predicate, compact, topology, include_properties)
...
def vector2mgrs_cli():
    ...
    parser.add_argument('-c', '--compact', action='store_true', help='Enable MGRS compact mode for polygons')
