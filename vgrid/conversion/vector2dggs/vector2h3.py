import os, sys, shutil, argparse, traceback, json
...
from vgrid.utils.geometry import densify_line, geodesic_buffer, geodesic_distance,check_predicate
...
def point2h3(resolution, point, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):        
    """
...
def polyline2h3(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):  
    """
    ...
    for polyline in polylines:    
        ...
        segment_length_degrees, _ = geodesic_distance(feature_lat, feature_lon, avg_edge_length)
        ...
        for h3_cell in h3_cells:
            ...
            if (h3.is_pentagon(h3_cell)):
                num_edges = 5           
            h3_feature = geodesic_dggs_to_feature("h3",h3_cell,cell_resolution,cell_polygon,num_edges)   
            if include_properties and feature_properties:
                h3_feature["properties"].update(feature_properties)
            h3_features.append(h3_feature)               
...
def polygon2h3(resolution, feature, feature_properties=None, predicate = None, compact= False, topology=False, include_properties=True):
    """
...
def geometry2h3(geometries, resolution, properties_list=None, predicate=None, compact=False, topology=False, include_properties=True):
    """
    ...
    for i, geom in enumerate(tqdm(geometries, desc="Processing features")):
        ...
        if geom.geom_type == 'Point':
            point_features = point2h3(resolution, geom, props, predicate, compact, topology, include_properties)
            h3_features.extend(point_features['features'])
            
        elif geom.geom_type == 'MultiPoint':
            for point in geom.geoms:
                point_features = point2h3(resolution, point, props, predicate, compact, topology, include_properties)
                h3_features.extend(point_features['features'])
                
        elif geom.geom_type in ['LineString', 'MultiLineString']:
            polyline_features = polyline2h3(resolution, geom, props, predicate, compact, topology, include_properties)
            h3_features.extend(polyline_features['features'])
                
        elif geom.geom_type in ['Polygon', 'MultiPolygon']:
            poly_features = polygon2h3(resolution, geom, props, predicate, compact, topology, include_properties)
...
def dataframe2h3(df, resolution, predicate = None, compact=False, topology=False, include_properties=True):
    """
    ...
    for col in df.columns:
        if hasattr(df[col].iloc[0], 'geom_type') or hasattr(df[col].iloc[0], '__geo_interface__'):
            geometry_col = col
            break
    
    if geometry_col is None:
        raise ValueError("DataFrame must contain a geometry column with Shapely geometry objects")
    ...
    return geometry2h3(geometries, resolution, properties_list, predicate, compact, topology, include_properties)

def geodataframe2h3(gdf, resolution, predicate = None, compact=False, topology=False, include_properties=True):
    """
...
def vector2h3(data, resolution, predicate=None, compact=False, topology=False, output_format='geojson', output_path=None, include_properties=True, **kwargs):
    """
    ...
    if hasattr(data, 'geometry') and hasattr(data, 'columns'):
        ...
        try:
            ...
            result = geodataframe2h3(gdf, resolution, predicate, compact, topology, include_properties)
...
def vector2h3_cli():
    ...
    parser.add_argument('-r', '--resolution', type=int, choices=range(16), metavar='[0-15]',
