...
from vgrid.conversion.vector2dggs.vector2h3 import (
    vector2h3, validate_h3_resolution,         
    point2h3, polyline2h3, polygon2h3
)
...
def test_all_input_types():
    ...
    test_data = {
        ...
        "multilinestring": MultiLineString([
            [(106.68, 10.77), (106.69, 10.78)],
            [(106.67, 10.76), (106.70, 10.79)]
        ]),
        "polygon": Polygon([(106.67, 10.76), (106.69, 10.76), (106.69, 10.78), (106.67, 10.78), (106.67, 10.76)]),
        "multipolygon": MultiPolygon([
            Polygon([(106.67, 10.76), (106.69, 10.76), (106.69, 10.78), (106.67, 10.78), (106.67, 10.76)]),
            Polygon([(106.70, 10.79), (106.72, 10.79), (106.72, 10.81), (106.70, 10.81), (106.70, 10.79)])
        ])
    }
    ...
    try:
        df_data = {
            ...
            'geometry': [test_data["point"], test_data["linestring"], test_data["polygon"]]
        }
        ...
        geojson_data = {
            ...
            "features": [
                ...
                },
                {
                    ...
                    "geometry": {
                        "type": "Polygon",
                        "coordinates": [[[106.67, 10.76], [106.69, 10.76], [106.69, 10.78], [106.67, 10.78], [106.67, 10.76]]]
                    },
...
def test_all_output_formats():
    ...
    # Test geometry
    test_geom = Polygon([(106.67, 10.76), (106.69, 10.76), (106.69, 10.78), (106.67, 10.78), (106.67, 10.76)])
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='shapefile', output_path=output_path)
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='gpkg', output_path=output_path)
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='csv', output_path=output_path)
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='parquet', output_path=output_path)
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='wkt', output_path=output_path)
        ...
        try:
            ...
            result = vector2h3(test_geom, resolution=9, output_format='wkb', output_path=output_path)
...
def test_predicates():
    ...
    # Test polygon
    test_polygon = Polygon([(106.67, 10.76), (106.69, 10.76), (106.69, 10.78), (106.67, 10.78), (106.67, 10.76)])
...
def test_resolutions():
    ...
    for resolution in [0, 5, 9, 12, 15]:
        try:
            ...
            print(f"  ✓ Resolution {resolution}: Generated {len(result['features'])} features")
        except Exception as e:
...
def test_compact_mode():
    ...
    try:
        ...
        if len(result_compact['features']) <= len(result_normal['features']):
            print(f"    ✓ Compact mode reduced features as expected")
        else:
            print(f"    ⚠ Compact mode did not reduce features")
...
def test_error_handling():
    ...
    try:
        result = vector2h3(Point(106.68, 10.77), resolution=20)
        print(f"  ✗ Should have failed for invalid resolution 20")
    ...
    try:
        result = vector2h3("invalid_input", resolution=9)
        print(f"  ✗ Should have failed for invalid input type")
    except ValueError as e:
    ...
    try:
        result = vector2h3(Point(106.68, 10.77), resolution=9, output_format='invalid_format')
        print(f"  ✗ Should have failed for invalid output format")
    except ValueError as e:
